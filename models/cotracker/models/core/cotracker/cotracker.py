...
class CoTracker(nn.Module):
    ...
    def forward(self, rgbs, queries, iters=4, feat_init=None, is_train=False):
        ...
        while ind < T - self.S // 2:
            ...
            # fmap_vis0 = fmaps[0, 0,...]
            # fmap_vis1 = fmaps[0, torch.randint(1,8,(1,))[0], ...]
            # fmap_vnorm0 = (
            #     (fmap_vis0-fmap_vis0.min())/
            #     (fmap_vis0.max()-fmap_vis0.min()))
            # fmap_vnorm1 = (
            #     (fmap_vis1-fmap_vis1.min())/
            #     (fmap_vis1.max()-fmap_vis1.min()))
            # H_vis, W_vis = fmap_vis0.shape[1:]
