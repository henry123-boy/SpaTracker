...
from spatracker1.utils.visualizer import Visualizer
    ...
    def evaluate_sequence(
        ...
        for ind, sample in enumerate(tqdm(test_dataloader)):
            ...
            if dataset_name == "badja" or dataset_name == "fastcapture":
                ...
                seq_name = str(ind)
            _, _, N, _ = sample.trajectory.shape

            vis.visualize(
                sample.video,
                pred_tracks[0][..., :2][...,:N, :] if isinstance(pred_tracks, tuple) else pred_tracks,
                filename=dataset_name + "_" + seq_name,
                writer=writer,
                step=step,
                gt_tracks=sample.trajectory[..., :N, :2],
            )
